...
window.onload = function() {
    auth_btn.addEventListener("click", () => {
        if (auth_name.value !== "" && auth_nickname.value !== "") {
            ...
            chat._nickname = auth_nickname.value.toString().toLowerCase();

            chat.connect(
                auth_name.value,
                auth_nickname.value.toString().toLowerCase()
            );
...
const chat = {
    ...
    connect: (name, nickname) => {
        chat._ws = createWS(chat._url, {
            newMessage: (data, ws) => {
                container.innerHTML =
                    templateMessages({
                        ...
                        timestamp: data.message.dateString.slice(
                            data.message.dateString.lastIndexOf(" "),
                            data.message.dateString.length
                        ),
                        self: data.message.nickname === chat._nickname,
                        avatar_url: `http://localhost:8081/photos/${
            ...
            getMessages: (data, ws) => {
                ...
                data.messages.forEach(message => {
                    container.innerHTML =
                        templateMessages({
                            text: message.text,
                            timestamp: message.dateString.slice(
                                message.dateString.lastIndexOf(" "),
                                message.dateString.length
                            ),
            ...
            refreshUsers: (data, ws) => {
                ...
                userlist.innerHTML = "";
                data.forEach(user => {
                    userlist.innerHTML += templateUserList({
                        name: user.name,
                        last_message: user.last_message,
                        avatar_url: `http://localhost:8081/${user.avatar ||
              "photos/no_avatar"}?${Date.now()}`
                    });
                });
                document.querySelector("#user_num").innerHTML = `${data.length} ${
          data.length === 1 ||
          (data.length % 10 === 1 && data.length % 100 !== 11)
            ? "участник"
            : data.length % 10 === 0 ||
              data.length % 10 >= 5 ||
              data.length % 100 === 11
            ...
            : "участника"
                ...
                let userProfileImg =
                    "http://localhost:8081/photos/" + chat._nickname + "?" + Date.now();
        ...
        chat._ws.onopen = function() {
            ...
            msg_btn.addEventListener("click", () => {
                if (input.value !== "") chat.sendMessage(input.value);

                input.value = "";
            });
        };
        chat._ws.onclose = function() {
            clearTimeout(chat.pingTimeout);
        };
    ...
    heartbeat: () => {
        clearTimeout(chat.pingTimeout);
        chat.pingTimeout = setTimeout(() => {
            chat._ws.close();
        }, 1000 + 500); // Возможный пинг до 500мс
    }
};